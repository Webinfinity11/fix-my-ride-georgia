...
const MechanicReviews = ({ mechanicId, onReviewAdded }: MechanicReviewsProps) => {
  const { user } = useAuth();
  ...
  const fetchReviews = async () => {
    console.log("📋 Fetching reviews for mechanic:", mechanicId);
    setLoading(true);
    ...
    try {
      ...
      // Check if current user has already reviewed this mechanic
      if (user) {
  ...
  const handleSubmitReview = async () => {
    ...
    console.log("📝 Submitting review:", { mechanicId, rating: newRating, comment: newComment });
    setSubmitting(true);
