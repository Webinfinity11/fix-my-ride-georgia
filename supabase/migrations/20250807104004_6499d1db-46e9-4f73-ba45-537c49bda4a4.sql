-- Smart Slug Generation System for mechanic_services

-- First, add a column to track if slug was manually set
ALTER TABLE mechanic_services 
ADD COLUMN IF NOT EXISTS slug_is_manual boolean DEFAULT false;

-- Create improved Georgian to Latin transliteration function
...
-- Create trigger function for smart slug generation
...
DROP TRIGGER IF EXISTS smart_slug_generation_trigger ON mechanic_services;

-- Create the trigger
CREATE TRIGGER smart_slug_generation_trigger
    BEFORE INSERT OR UPDATE ON mechanic_services
    FOR EACH ROW
    EXECUTE FUNCTION smart_slug_trigger();

-- Update existing records that have NULL slugs
UPDATE mechanic_services 
SET slug = generate_unique_slug(name, id), slug_is_manual = false
WHERE slug IS NULL OR slug = '';

-- Update slug_is_manual for existing records (assume auto-generated)
UPDATE mechanic_services 
SET slug_is_manual = false 
WHERE slug_is_manual IS NULL;