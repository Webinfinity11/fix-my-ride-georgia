-- Create database triggers for automatic sitemap updates (fixed version)
...
-- Trigger for profiles table (for mechanic role changes)
DROP TRIGGER IF EXISTS sitemap_update_mechanics ON profiles;
CREATE TRIGGER sitemap_update_mechanics
  AFTER INSERT OR UPDATE OF role, is_verified OR DELETE ON profiles
  FOR EACH ROW
  EXECUTE FUNCTION trigger_sitemap_update();